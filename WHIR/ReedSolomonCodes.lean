...
Given a finite field `F`, a nonempty set of evaluation points `L ⊆ F`, and a degree bound `d`, we construct the Reed–Solomon code
as follows:

1. **Polynomials of bounded degree.** We collect all univariate polynomials over `F` of degree < `d` into the finite set `polynomialsUpTo F d : Finset (Polynomial F)` by identifying each polynomial with its list of coefficients in Fin d → F.

2. **Evaluation map.** Each polynomial `p` in `polynomialsUpTo F d` is evaluated at every point in `L`, yielding a function `L → F`, i.e `polynomialEvalOn F L : Polynomial F → (↑L → F)`

3. **Reed–Solomon code.** The set of codewords is defined as the image of the above evaluation map.

The construction ensures that C.code is constructive and of Fintype `Finset (L → F)`, which makes constructions like filtering and Lagrange interpolation more convinient.
...
/-- Given a codeword f ∈ C.code, C.poly(f) computes an associated polynomial \hat{f} with \hat{f}(l) = f(l) ∀ l ∈ L and deg(\hat{f}) < d-/
